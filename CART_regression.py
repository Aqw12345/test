# -*- coding: utf-8 -*-
"""
Created on Thu Aug  2 16:46:32 2018
...
def choseBestFeature(dataSet,op = [1,4]):          # 三个停止条件可否当作是三个预剪枝操作
    ...
    BestFeature = -1; BestNumber = 0; lowError = np.inf
    ...
    for i in range(n-1):    # 遍历每一个特征值
        for j in set(dataSet[:,i].T.tolist()[0]):
            ...
            if np.shape(dataR)[0]<op[1] or np.shape(dataL)[0]<op[1]: continue  # 如果所给的划分后的数据集中样本数目甚少，则直接跳出
...
def forecastSample(Tree,testData):
    if not isTree(Tree): return float(np.tree)
